...
int main(){
	...
	rxcfg.lo_hz = MHZ(2500); // 2.5 GHz rf frequency
	...
	txcfg.lo_hz = MHZ(2500); // 2.5 GHz rf frequency
...
// The cf-ad9361-dds-core-lpc is the DAC/TX output driver that controls the TX DMA and the TX HDL core, including the DDS.
    ...
    memset(rx_i, 0, sizeof(rx_i));
    memset(rx_q, 0, sizeof(rx_q));

    int32_t counter = 0;
    while (counter < num_samples -1) {
        // Получаем блоки для RX и TX
        ...
        for (p_dat = static_cast<int16_t *>(iio_block_first(txblock, tx0_i)); p_dat < p_end; p_dat += p_inc / sizeof(*p_dat)) {
            if (counter < num_samples) {
                p_dat[0] = tx_i[counter] /10; // I
                p_dat[1] = tx_q[counter] /10; // Q
            } else {
                num_samples = 0;
            }
        }
        ...
        for (p_dat = static_cast<int16_t *>(iio_block_first(rxblock, rx0_i)); p_dat < p_end; p_dat += p_inc / sizeof(*p_dat)) {
            rx_i[sample_index] = p_dat[0]; // I
            rx_q[sample_index] = p_dat[1]; // Q
            sample_index++;
        }
