...
def plot_eye_diagram(signal, sps, num_traces=100, save_path=None):
...
# Путь к файлам и папка для графиков
filename = '../../data/new/txdata2.pcm'
save_dir = 'plots_timing_recovery'
...
plt.grid(True)
plt.savefig(os.path.join(save_dir, 'original_signal.png'))
plt.close()
...
plt.axis('equal')
plt.grid(True)
plt.savefig(os.path.join(save_dir, 'original_constellation.png'))
plt.close()
...
plt.grid(True)
plt.savefig(os.path.join(save_dir, 'filtered_signal.png'))
plt.close()
...
plt.axis('equal')
plt.grid(True)
plt.savefig(os.path.join(save_dir, 'filtered_constellation.png'))
plt.close()
...
if len(IQs) > 0 and len(ns) > 0:
    ...
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, 'synced_signal.png'))
    plt.close()
    ...
    plt.axis('equal')
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, 'synced_constellation.png'))
    plt.close()
...
if len(err) > 0:
    ...
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, 'error_plot.png'))
    plt.close()
...
if len(offset) > 0:
    ...
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, 'offset_plot.png'))
    plt.close()

plt.tight_layout()

# Дополнительно сохраним eye diagram
plot_eye_diagram(IQ_filtered, Nsps, num_traces=100, save_path=os.path.join(save_dir, 'eye_diagram.png'))

# Если хочешь, можешь убрать plt.show(), чтобы не открывать окна
