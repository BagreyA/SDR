...
def plot_cross_correlation(x_fixed, x_data, filename=None):
    n_fixed = len(x_fixed)
    n_data = len(x_data)
    correlations = []
...
def plot_autocorrelation(data, filename=None):
    n = len(data)
    mean = np.mean(data)
    var = np.var(data)
...
def autocorrelation_and_plot(seq, filename=None):
    n = len(seq)
    result = np.zeros(n)
...
def plot_signal(real_part, imag_part, filename_prefix=None):
    ...
    plt.legend()
    if filename_prefix:
        plt.savefig(f'{filename_prefix}_real.png')
        plt.close()
    else:
        plt.show()
    ...
    plt.legend()
    if filename_prefix:
        plt.savefig(f'{filename_prefix}_imag.png')
        plt.close()
    else:
        plt.show()
...
plot_signal(real_part, imag_part, filename_prefix=os.path.join(output_dir, 'qpsk_signal'))
