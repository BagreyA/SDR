...
private:
    static const unsigned int LENGTH = 40000; //Вот тут был мой косяк (размер очереди был слишком мал)
...
template class AtomicQueue<unsigned char>;
template class AtomicQueue<int>;
...
template class AtomicQueue<double>;   //явно создаем экземпляр шаблона класса