...
def plot_eye_diagram(signal, sps, num_traces=100):
    ...
    for i in range(num_traces):
        ...
        if start_idx + 2 * sps > len(signal):
            break
        plt.plot(range(2 * sps), signal[start_idx:start_idx + 2 * sps].real, color='blue')
...
plt.subplot(323)
plt.plot(filtered_I, label='Filtered I')
plt.plot(filtered_Q, label='Filtered Q')
plt.title('Отфильтрованные данные I и Q')
plt.legend()
plt.grid(True)

plt.subplot(324)
plt.scatter(filtered_I, filtered_Q)
